...
static inline struct bstring create_key(uint64_t _int)
{
    ...
    cc_snprintf(val_str, len + 1, "%lu", _int);
...
static inline struct bstring create_val(uint64_t length)
{
    struct bstring ret_val;
    ret_val.data = cc_alloc(length);
    cc_memset(ret_val.data, 'A', length);
...
check_consistency(struct bstring *keys, uint32_t slab_nitem, uint32_t item_size)
{
    ...
    for (i = 0; i < slab_nitem; i++) {
        ...
        if (it == NULL) {
            ...
            data = NULL;
            data = item_data(it);
            val_size = item_size - item_ntotal(keys->len, 0, 0);
            val_pattern = cc_alloc(val_size);
            memset(val_pattern, 'A', val_size);
...
test_assert_crud_multiple_keys(uint32_t slab_mem, uint32_t slab_nitem, uint32_t item_size)
{
    ...
    for (i = 0; i < slab_nitem; i++) {
        ...
        keys[i] = create_key(i);
        val = create_val(item_size - item_ntotal(keys[i].len, 0, 0));
...
{
    struct params p[3] = { {100*MiB, 100000, 64}, { 100*MiB, 50000, KiB}, { 10*MiB, 13, MiB/3 }};
