...
{
    if (cc_memcmp(((struct bstring *)key1)->data, ((struct bstring *)key2)->data, ((struct bstring *)key1)->len) == 0) {
        return true;
    }
...
check_consistency(struct bstring *keys, uint32_t nkey, uint32_t item_size, struct array *keys_deleted, struct array *keys_updated)
{
    ...
    for (i = 0; i < nkey; i++) {
        ...
        if (it == NULL) {
            ck_assert_msg(array_each(keys_deleted, are_keys_equal, &keys[i], &err) < nkey,
            ...
            cc_snprintf(val_pattern, val_len + 1, "%.*u", val_len, i);
             ...
             if (cc_memcmp(data, val_pattern, val_len) != 0) {
                ck_assert_msg(array_each(keys_updated, are_keys_equal, &keys[i], &err) < nkey,
                ...
                ck_assert_msg(cc_memcmp(UPDATED_VAL, data, sizeof(UPDATED_VAL) - 1) == 0,
                             "item with key %.*s was updated with %s, but current value is %s",
                              keys[i].len, keys[i].data, UPDATED_VAL, item_data(it));
            }
            free(val_pattern);
        }
    }
...
test_assert_crud_multiple_keys(uint32_t slab_mem, uint32_t nkey, uint32_t item_size)
{
    ...
    array_create(&keys_deleted, 100, sizeof(struct bstring));
    array_create(&keys_updated, 100, sizeof(struct bstring));
